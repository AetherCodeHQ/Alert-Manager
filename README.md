...
> Alert-Manager - Open source tool by AetherCodeHQ

`alerts` `cli` `devops` `golang` `monitoring` `open-source`

## What is it?

**Alert Manager** is a devops tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Alert-Manager --path ./target

# With options
./Alert-Manager --path ./target --format json --output report.json

# Verbose mode
./Alert-Manager --path ./target --verbose
```
...
### Basic scan
```bash
./Alert-Manager --path ./my-project
```

### JSON report
```bash
./Alert-Manager --path ./my-project --format json --output report.json
```
...
- name: Run Alert Manager
  run: ./Alert-Manager --path . --format json --output report.json
```
